  ...
  >
    ...
    `}_renderCrossHair(e){if(this._dragCurveIdx<0)return W;const t=e.controlPoints[this._dragPointIdx];if(!t)return W;const i=Re(t.lightener),r=Ue(t.target);return q`
      ...
      <line class="crosshair"
        ...
        x2="${Te}" y2="${r}"
        stroke="${e.color}" opacity="0.5" />
    `}_renderTooltip(e){const t=Re(e.lightener),i=Ue(e.target),r=`Group ${e.lightener}% -> Light ${e.target}%`,n=Math.ceil(4.9*r.length),s=Fe(t-n/2-2,Te,344-n-8),o=Math.max(16,i-16);return q`
    ...
    `,r=q`
      <line class="scrubber-line"
        x1="${t}" y1="${Ue(0)}"
        x2="${t}" y2="${Ue(100)}" />
    `,n=this.curves.filter(e=>e.visible).map(i=>{const r=Ue(Ne(i.controlPoints,e));return q`
          <circle
            class="scrubber-dot"
            cx="${t}" cy="${r}"
            r="4"
        ...
        <path
          ...
          pointer-events="none"
        />
      `}catch{return W}}_renderCurvePoints(e,t){if(!e.visible||!e.controlPoints.length)return W;try{const i=this._isCurveInteractive(t);if(!(i&&!this.readOnly))return W;const r=this._dragCurveIdx===t,n=e.color+"33";let s=null;if(r&&this._dragPointIdx>=0)s=e.controlPoints[this._dragPointIdx];else if(this._hoveredPoint?.curve===t||this._focusedPoint?.curve===t){const i=this._focusedPoint?.curve===t?this._focusedPoint.point:this._hoveredPoint?.point??-1;s=e.controlPoints[i]??null}return q`
        ${e.controlPoints.map((i,s)=>{const o=0===s,a=r&&this._dragPointIdx===s,l=this._hoveredPoint?.curve===t&&this._hoveredPoint?.point===s,d=null!==this.scrubberPosition&&i.lightener>this.scrubberPosition?.35:1;return q`
            <circle
            ...
            />
            ${function(e,t,i,r,n,s,o,a,l){const d=`--glow-color: ${l}; opacity: ${a}`;switch(e){case"circle":return q`<circle
        class="${n}"
        cx="${t}"
        cy="${i}"
        r="${r}"
    ...
    `}render(){const e=function(e){if(e.length<2)return e.map(e=>({prefix:"",discriminator:e}));let t=e[0].length;for(let i=1;i<e.length;i++){const r=e[0],n=e[i];let s=0;for(;s<t&&s<n.length&&r.charCodeAt(s)===n.charCodeAt(s);)s++;if(t=s,0===t)break}const i=e[0];for(;t>0&&!gt.test(i[t-1]);)t--;let r=t;for(;r>0&&gt.test(i[r-1]);)r--;if(0===r)return e.map(e=>({prefix:"",discriminator:e}));const n=i.slice(0,r),s=e.map(e=>({prefix:n,discriminator:e.slice(t).replace(/^[\s\-–—/:]+/,"")}));return s.some(e=>0===e.discriminator.length)?e.map(e=>({prefix:"",discriminator:e})):s}(this.curves.map(e=>e.friendlyName)),t=this.curves.filter(e=>e.visible).length,i=this.curves.length-t,r=0===this.curves.length?et.emptyCount:0===i?et.countAllVisible(this.curves.length):et.countWithHidden(t,i),n=this.curves.length>=20;return V`
      ...
      >
        ...
        <div class="legend" role="list" aria-label=${et.listAria(this.curves.length)}>
          ${this.curves.map((t,i)=>{const r=this.canManage&&!this.managing&&this._confirmingRemove===t.entityId,n=e[i],s=this.selectedCurveId===t.entityId;return V`
              <div
                ...
                ${r?this._renderConfirmRow(t):V`
                      ...
                      >
                        ...
                        <span class="name-block">
                          ...
                          >
                          ${n.prefix?V`<span class="prefix">${n.prefix}</span>`:W}
...
>
  ...
  `,e([ve()],mt.prototype,"_config",void 0),e([ve()],mt.prototype,"_hass",void 0),be("lightener-curve-card-editor",mt);class bt extends ce{constructor(){super(...arguments),this._curves=[],this._originalCurves=[],this._config={},this._selectedCurveId=null,this._saveState=st,this._load=at,this._manageError=null,this._managingLights=!1,this._groupDeleted=!1,this._scrubberPosition=null,this._cancelAnimating=!1,this._hass=null,this._undoStack=[],this._dragUndoPushed=!1,this._dragActive=!1,this._autoPresetsShownFor=new Set,this._boundKeyHandler=null,this._boundBeforeUnload=null,this._saveGuard=new ot({dispatchSave:e=>this._dispatchSave(e),getSavePhase:()=>this._saveState.phase}),this._cancelAnimFrame=null,this._previewActive=!1,this._showPresets=!1,this._presetGraphTrial=null,this._legendCloseRemoveSignal=0,this._legendCloseAddSignal=0,this._manageMode=!1,this._freshPresetTrialEntityId=null,this._freshPresetAutoSelectedCurveId=null,this._lastPresetPointerType=null,this._previewController=new Ve({getHass:()=>this._hass,getCurves:()=>this._curves,getScrubberPosition:()=>this._scrubberPosition,setScrubberPosition:e=>{this._scrubberPosition=e},getStorageEntityId:()=>this._storageEntityId,persistScrubberPosition:(e,t)=>{this._writeStoredState(e,{scrubberPosition:t})},setPreviewActive:e=>{this._previewActive=e}}),this._lastEmittedDirtyState=!1,this._dirtyVersion=0,this._cleanVersion=0,this._onPreviewToggle=()=>{this._clearFreshPresetTrial(),this._previewActive?this._stopPreview():this._startPreview()},this._startPreview=()=>{this._previewController.start()},this._stopPreview=()=>{this._previewController.stop()}}get _saving(){return"saving"===(e=this._saveState).phase||"confirming"===e.phase;var e}get _saveSuccess(){return"saved"===this._saveState.phase}get _saveError(){return"error"===(e=this._saveState).phase?e.message:null;var e}_dispatchSave(e){const t="confirming"===this._saveState.phase;this._saveState=function(e,t){switch(t.type){case"reset":return{phase:"idle"};case"dirty":return"idle"===e.phase?{phase:"dirty"}:e;case"save-start":return"saving"===e.phase||"confirming"===e.phase?e:{phase:"saving"};case"save-success":return"saving"!==e.phase?e:{phase:"confirming"};case"save-confirmed":return"confirming"!==e.phase?e:{phase:"saved"};case"save-error":return"saving"!==e.phase&&"confirming"!==e.phase?e:{phase:"error",message:t.message};case"save-clear":return"saved"===e.phase||"error"===e.phase?{phase:"idle"}:e}}(this._saveState,e),"confirming"!==this._saveState.phase&&this._saveGuard.onLeaveConfirming(this._saveState.phase,t)}get _lastPreviewTime(){return this._previewController.lastPreviewTime}set _lastPreviewTime(e){this._previewController.lastPreviewTime=e}get _embedded(){return!0===this._config.embedded}static getConfigElement(){return document.createElement("lightener-curve-card-editor")}static getStubConfig(){return{type:fe}}setConfig(e){const t=e.entity!==this._config.entity;this._config=e,t&&(this._previewActive&&this._stopPreview(),this._dragActive=!1,this._load=function(e){return{...e,loaded:!1,loadedEntityId:void 0,loadError:null,loadErrorEntityId:void 0,pendingReloadEntityId:void 0,reloadAfterLoadEntityId:void 0}}(this._load),this._groupDeleted=!1,this._clearFreshPresetTrial(),this._showPresets=!1,this._selectedCurveId=null,this._scrubberPosition=null,this._undoStack=[],this._cleanVersion=this._dirtyVersion,this._tryLoadCurves())}set hass(e){const t=!!this._hass;this._hass=e,t&&this._load.loaded||this._dragActive||this._tryLoadCurves()}getCardSize(){return 4}getGridOptions(){return{columns:12,rows:9,min_columns:6,min_rows:6}}get _isAdmin(){return this._hass?.user?.is_admin??!1}get _entityId(){return this._config.entity}get _storageEntityId(){return this._load.loadedEntityId??this._entityId}get _isDirty(){return this._dirtyVersion!==this._cleanVersion}get _selectedCurve(){if(null!==this._selectedCurveId)return this._curves.find(e=>e.entityId===this._selectedCurveId)}get _canShowPresetGraphTrial(){return this._showPresets&&!this._isDirty&&!this._saving&&!this._cancelAnimating&&!this._managingLights&&!this._previewActive&&null!==this._freshPresetTrialEntityId&&this._freshPresetTrialEntityId===this._storageEntityId&&function(e){const t=e.filter(e=>!1!==e.visible);return Ke(t)}(this._curves)}get _isShowingPresetGraphTrial(){return null!==this._presetGraphTrial&&this._canShowPresetGraphTrial}get _graphCurves(){return this._presetGraphTrial&&this._canShowPresetGraphTrial?Ce(this._curves,this._selectedCurveId,this._presetGraphTrial.controlPoints):this._curves}get _canManageLights(){return this._isAdmin&&!!this._hass&&!!this._entityId&&!this._isDirty&&!this._saving&&!this._cancelAnimating&&!this._load.loading&&!this._managingLights&&!this._load.loadError&&!this._groupDeleted}get dirty(){return this._isDirty}connectedCallback(){super.connectedCallback(),this._load.loadErrorEntityId!==this._entityId&&(this._load=dt(this._load)),this._groupDeleted&&this._load.loadedEntityId!==this._entityId&&(this._groupDeleted=!1,this._load=dt(this._load)),this._tryLoadCurves(),this._boundKeyHandler=this._onKeyDown.bind(this),this._boundBeforeUnload=this._onBeforeUnload.bind(this),window.addEventListener("keydown",this._boundKeyHandler),window.addEventListener("beforeunload",this._boundBeforeUnload)}disconnectedCallback(){super.disconnectedCallback(),this._previewActive&&this._stopPreview(),this._clearFreshPresetTrial(),this._previewController.disconnect(),this._dragActive=!1,this._boundKeyHandler&&window.removeEventListener("keydown",this._boundKeyHandler),this._boundBeforeUnload&&window.removeEventListener("beforeunload",this._boundBeforeUnload),"confirming"===this._saveState.phase&&(this._saveGuard.settleError(),this._load={...this._load,loading:!1,loaded:!1,reloadAfterLoadEntityId:void 0},this._dispatchSave({type:"reset"})),this._saveGuard.dispose(),this._cancelAnimFrame&&(cancelAnimationFrame(this._cancelAnimFrame),this._cancelAnimFrame=null,this._cancelAnimating=!1)}updated(e){if(super.updated(e),e.has("_curves")||e.has("_originalCurves")||e.has("_cancelAnimating")){const e=this._isDirty;e!==this._lastEmittedDirtyState&&(this._lastEmittedDirtyState=e,this.dispatchEvent(new CustomEvent("curve-dirty-state",{detail:{dirty:e},bubbles:!0,composed:!0})),e&&this._dispatchSave({type:"dirty"}))}}_clearPresetGraphTrial(){this._presetGraphTrial=null}_freshGroupTrialPreset(){return qe.find(e=>"dim_accent"===e.id)??qe.find(e=>"linear"!==e.id)??null}_setFreshGroupTrialPreset(){this._presetGraphTrial=this._freshGroupTrialPreset()}_firstVisibleCurveId(e){return e.find(e=>!1!==e.visible)?.entityId??null}_startFreshPresetTrial(e,t){const i=this._firstVisibleCurveId(t);this._showPresets=!0,this._freshPresetTrialEntityId=e,this._freshPresetAutoSelectedCurveId=i,this._selectedCurveId=i,this._setFreshGroupTrialPreset()}_freshPresetTrialIsActive(){return null!==this._freshPresetTrialEntityId&&this._freshPresetTrialEntityId===this._storageEntityId&&this._showPresets}_clearFreshPresetTrial(e=!0){e&&null!==this._freshPresetAutoSelectedCurveId&&this._selectedCurveId===this._freshPresetAutoSelectedCurveId&&(this._selectedCurveId=null),this._freshPresetTrialEntityId=null,this._freshPresetAutoSelectedCurveId=null,this._lastPresetPointerType=null,this._clearPresetGraphTrial()}_promoteAuditionToUserSelection(){this._freshPresetAutoSelectedCurveId=null,this._presetGraphTrial||this._setFreshGroupTrialPreset()}_rememberPresetPointer(e){this._lastPresetPointerType=e.pointerType}_startPresetGraphTrial(e,t){"touch"!==(t&&"pointerType"in t?String(t.pointerType):this._lastPresetPointerType)&&this._canShowPresetGraphTrial&&(this._presetGraphTrial=e)}_endPresetGraphTrial(e){this._presetGraphTrial?.id===e.id&&(this._freshPresetTrialIsActive()?this._setFreshGroupTrialPreset():this._clearPresetGraphTrial()),this._lastPresetPointerType=null}_togglePresets(){if(this._managingLights)return;if(0===this._curves.length)return;const e=!this._showPresets;this._showPresets=e,e||this._clearFreshPresetTrial(),e&&(this._legendCloseAddSignal++,this._legendCloseRemoveSignal++)}_onLegendRemovePanelOpen(){this._clearFreshPresetTrial(),this._showPresets=!1}_applyPreset(e){if(this._cancelAnimating||this._saving||this._managingLights)return;if(0===this._curves.length)return;const t=this._selectedCurveId,i=this._freshPresetTrialIsActive(),r=Ce(this._curves,t,e.controlPoints);this._clearFreshPresetTrial(!1),this._showPresets=!1,we(r,this._curves)?i&&(this._selectedCurveId=null):this._commitCurveEdit(r)}_renderPresetsPanel(){const e=null!==this._selectedCurveId?Qe.targetLight(this._selectedCurve?.friendlyName??"selected light"):Qe.targetAll;return V`
    ...
    `}_renderGraphInsight(){if(this._isShowingPresetGraphTrial&&this._presetGraphTrial){const e=this._selectedCurve;return V`
      ...
      `}const e=Ye(this._curves,this._selectedCurveId);return e?V`
